 ...
 *
 * $Id: tal_fe.c 37632 2015-07-14 14:39:39Z uwalter $
 *
...
#    define bb_blk_write(reg, addr, len)    pal_trx_write(reg, addr, len)
...
retval_t ofdm_rfcfg(ofdm_option_t ofdm_opt, trx_id_t trx_id)
{
    ...
    /* Note: all settings here are sourced from Datasheet Table 6-88 'Recommended Transmitter Frontend Configuration'
	and Table 6-91 'Recommended PHY Receiver and Digital Frontend Configuration' */
    switch (ofdm_opt)
    {
...
retval_t fsk_rfcfg(fsk_mod_type_t mod_type, fsk_sym_rate_t srate, mod_idx_t mod_idx, trx_id_t trx_id)
{
    ...
    /* - Transmit Power*/
    pal_dev_reg_write(RF215_TRX, GET_REG_ADDR(RG_RF09_PAC),
