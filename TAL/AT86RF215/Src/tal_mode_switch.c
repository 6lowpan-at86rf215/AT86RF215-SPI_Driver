 ...
 *
 * $Id: tal_mode_switch.c 37362 2015-05-11 13:44:53Z uwalter $
 *
...
//static void tx_actual_frame(timer_element_t *cb_timer_element);
static void tx_actual_frame(union sigval v);
static void configure_new_tx_mode(trx_id_t trx_id);
//static void cancel_new_mode_reception(timer_element_t *cb_timer_element);
...
static inline void download_ms_ppdu(trx_id_t trx_id)
{
    ...
    pal_dev_write(RF215_TRX, tx_frm_buf_offset + RG_BBC0_FBTXS, (uint8_t *)&phr, 2);
}
...
void tx_ms_ppdu(trx_id_t trx_id)
{
    ...
    CALC_REG_OFFSET(trx_id);
    pal_dev_reg_write(RF215_TRX, GET_REG_ADDR(RG_BBC0_AMCS), 0);
...
static void configure_new_tx_mode(trx_id_t trx_id)
{
    ...
    {
        CALC_REG_OFFSET(trx_id);
        pal_dev_bit_write(RF215_TRX, GET_REG_ADDR(SR_BBC0_AMCS_TX2RX), 1);
    }
...
void handle_rx_ms_packet(trx_id_t trx_id)
{
    ...
    pal_dev_read(RF215_TRX, rx_frm_buf_offset + RG_BBC0_FBRXS, (uint8_t *)&phr, 2);
